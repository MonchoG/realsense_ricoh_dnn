...
# https://github.com/autorope/donkeycar/blob/dev/donkeycar/parts/realsense435i.py
...
class RealSense435i:
    def __init__(self, width=WIDTH, height=HEIGHT, channels=CHANNELS, enable_rgb=True, enable_depth=True, enable_imu=False,record_bag=None, read_bag=None, device_id=None):
        ...
        if self.resize:
            print("The output images will be resized from {} to {}.  This requires opencv.".format(
    ...
    def _poll(self):
        ...
        if self.enable_rgb or self.enable_depth:
            ...
            aligned_frames = self.align.process(
                frames) if self.enable_depth and self.enable_rgb else None
            ...
            if self.resize:
                ...
                if self.width != WIDTH or self.height != HEIGHT:
                    self.color_image = cv2.resize(
                        self.color_image, (self.width, self.height), cv2.INTER_NEAREST) if self.enable_rgb else None
                    self.depth_image = cv2.resize(
                        self.depth_image, (self.width, self.height), cv2.INTER_NEAREST) if self.enable_depth else None
                if self.channels != CHANNELS:
                    self.color_image = cv2.cvtColor(
                        self.color_image, cv2.COLOR_RGB2GRAY) if self.enable_rgb else None
        ...
        #
        if self.enable_imu:
            acceleration = imu_frames.first_or_default(
            ...
            gyroscope = imu_frames.first_or_default(
